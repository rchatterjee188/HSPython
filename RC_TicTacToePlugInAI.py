...
def getComputerMove(board, computerLetter):
    ...
    while not free:
        if board[5] == ' ':
            move = 5
        elif checkCompChar(board, computerLetter) != 0 and isSpaceFree(board, checkCompChar(board, computerLetter)):
            move = checkCompChar(board, computerLetter)
        elif checkPlayerChar(board, computerLetter) != 0 and isSpaceFree(board, checkPlayerChar(board, computerLetter)):
            move = checkPlayerChar(board, computerLetter)
        else:
            move = random.randint (1,9)
        free = isSpaceFree(board, move)
    return move
    ...
    while True:
        ...
        while gameIsPlaying:
            if turn == 'player':
                ...
                theBoard = makeMove(theBoard, playerLetter, move)
